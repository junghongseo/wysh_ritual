...
def draft_deep_dive_article(fact_sheet: str, selected_title: str) -> str:
    ...
    5. 실용적인 가이드: 글의 마지막에 독자가 일상에서 이 리추얼을 실천해볼 수 있는 구체적인 행동 가이드를 넣으세요.
    6. 출처 링크 기재: 글 마지막에 본문에 활용한 실제 브랜드 공식 홈페이지나 기사 레퍼런스 URL들을 명확히 나열하세요.
    7. **한국 브랜드 제외**: 자사 비즈니스와의 충돌을 막기 위해 본문 내에 한국 로컬 브랜드나 한국 기업, 한국 시장에 대한 언급을 절대 금지합니다. 오직 해외 글로벌 브랜드만 다루십시오.
...
def review_and_revise_draft(draft: str, fact_sheet: str) -> str:
    ...
    for iteration in range(2): # 최대 2번만 재검수 (원본 1번 + 수정 최대 2번 = 총 3번)
        ...
        조금이라도 다양한 도시를 "수박 겉핥기" 식으로 나열했거나, 고유명사(메뉴명 등)를 부정확하게 기재했거나, 한국 브랜드가 포함되어 있다면 100% 반려(is_approved=false)해야 합니다.
