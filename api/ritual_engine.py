...
class EditorialContent(BaseModel):
    ...
    insta_carousel: str = Field(description="인스타그램 카드뉴스용 텍스트. 총 4~8장의 슬라이드로 구성하며, [Slide 1] [Slide 2] 처럼 명시할 것. 아티클의 깊이 있는 내용을 충분히 담을 수 있도록, **각 슬라이드는 타이틀 외에 본문 텍스트가 최대 100자 내외(띄어쓰기 제외)가 되도록 풍성하고 구체적으로 작성**할 것. 너무 짧은 단답형 문장을 지양하고, 반드시 한국어로 작성하며, **마지막 슬라이드에는 독자가 스스로 일상에서 어떻게 이 리추얼을 시도할 수 있는지 구체적인 실천 가이드(How to apply)**를 제안할 것.")
...
- **[매우 중요] 논리적 연결성 (No Clickbait)**: 카카오톡 프리뷰(Teaser)에서 제기한 질문이나 후킹용 소재를, 웹 아티클 본문 서두에서 반드시 가장 먼저, 상세히 논리적으로 설명하며 해소해야 합니다.
