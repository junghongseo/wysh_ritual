...
- **[매우 중요] 단일 주제 집중 (Single Theme Focus)**: 제공된 3개의 트렌드 소스 기사 내용들을 기계적으로 모두 섞어서 병렬적으로 나열하지 마십시오! 소스 기사들 중에서 **가장 흥미롭고 사람들의 공감을 이끌어낼 수 있는 단 하나의 핵심 트렌드/주제만**을 메인 테마로 선정하여 깊이 있게 파고들어야 합니다. (여러 기사가 주어지더라도 오직 하나의 소재에만 온전히 집중하여 몰입감 있고 일관된 하나의 완결된 스토리를 만드십시오.)
